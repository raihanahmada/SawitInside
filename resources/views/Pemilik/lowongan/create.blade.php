...
@section('title', 'Buat Lowongan — SAWIT INSIDE')

@section('pemilik_content')

<!-- 1. CSS CUSTOM UNTUK MODAL JAM -->
<style>
    .clockpicker-popover {
...
<div class="max-w-4xl mx-auto p-8 bg-white rounded-xl shadow-2xl my-10 border-t-4 border-emerald-600">
    ...
    <div class="mb-8 text-center">
        <h2 class="text-2xl font-bold text-emerald-800">Buat Lowongan Baru</h2>
        <p class="text-gray-500 text-sm mt-1">Lengkapi detail pekerjaan di bawah ini.</p>
    </div>
    ...
    <form action="{{ route('pemilik.lowongan.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" required placeholder="Contoh: Pemanen Sawit" class="w-full p-3 border border-gray-300 rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi</label>
                    <textarea name="deskripsi" rows="4" required class="w-full p-3 border border-gray-300 rounded-lg">{{ old('deskripsi') }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Lokasi Kerja</label>
                    <input type="text" name="lokasi_kerja" value="{{ old('lokasi_kerja') }}" placeholder="Contoh: Blok A" class="w-full p-3 border border-gray-300 rounded-lg">
                </div>
            </div>
        </div>
        ...
        <div class="bg-emerald-50 p-6 rounded-lg border border-emerald-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <select id="jenis_upah" name="jenis_upah" class="w-full p-3 border border-gray-300 rounded-lg">
                            <option value="">-- Pilih Sistem --</option>
                            <option value="borongan" {{ old('jenis_upah') == 'borongan' ? 'selected' : '' }}>Borongan</option>
                            <option value="harian" {{ old('jenis_upah') == 'harian' ? 'selected' : '' }}>Per Hari</option>
                            <option value="bulanan" {{ old('jenis_upah') == 'bulanan' ? 'selected' : '' }}>Per Bulan</option>
                            <option value="manual" {{ old('jenis_upah') == 'manual' ? 'selected' : '' }}>Manual</option>
                        </select>
                        <div id="nilai_wrapper" class="hidden">
                            <select id="nilai_upah" name="nilai_upah" class="w-full p-3 border border-gray-300 rounded-lg"></select>
                        </div>
                        <div id="manual_wrapper" class="hidden relative">
                            ...
                            <input type="number" id="nilai_manual" name="nilai_manual" class="w-full pl-10 p-3 border border-gray-300 rounded-lg" placeholder="0">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2 mb-2">
                        ...
                        <div>
                            ...
                            <div class="relative clockpicker">
                                <!-- PENTING: Tambahkan name="jam_pulang" -->
                                <input type="text" id="jam_pulang" name="jam_pulang"
                                    class="w-full p-2 border border-gray-300 rounded-lg bg-white cursor-pointer"
                                    readonly placeholder="Pilih..." value="{{ old('jam_pulang') }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_kebutuhan" value="{{ old('jumlah_kebutuhan', 1) }}" required min="1" class="w-full p-3 border border-gray-300 rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Batas Akhir Lamaran</label>
                    <input type="date" name="batas_pelamar" value="{{ old('batas_pelamar') }}" class="w-full p-3 border border-gray-300 rounded-lg">
                </div>
            </div>
        </div>

        <div class="flex items-center gap-4 pt-6 border-t border-gray-100">
            <button type="submit" class="flex-1 py-3 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg shadow-md transition duration-200">
                Simpan Lowongan
            </button>
...
<script>
    ...
    if (jenisEl) {
        ...
        nilaiSelect.addEventListener('change', function() {
            ...
            else manualWrapper.classList.add('hidden');
        });
    }
</script>
@endsection